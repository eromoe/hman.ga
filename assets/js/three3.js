...
// pre load numbers 
numImgName = 'hanumbers.png'
numImg.src = ImageFolder+ numImgName;
...
var numcopy = numImgName;

sitename = document.domain;

sitename= sitename == 'hman.ga' ? sitename :null; 

cdtempCubes = numcopy[0] +numcopy[4]

imageC = numcopy[1] + numcopy[11]
...
function makeLogoPlanes() { 
	...
	for(var i =0; i<(Detector.webgl?20:3); i++)
	{
		...
		material = new THREE.MeshBasicMaterial( {map: THREE.ImageUtils.loadTexture( ImageFolder+'CreativeJSwob2.png' ),  opacity:(i==0)?0.9:(i>=3)?0.012:0.1, blending :THREE.AdditiveBlending, depthTest:false,transparent:true});//(1-(i/4))*0.2});	
		//material = new THREE.MeshBasicMaterial( {map: THREE.ImageUtils.loadTexture( ImageFolder+'12daysTypeGlow.png' ),  opacity:(i==0)?0.9:(i>=3)?0.012:0.1, blending :THREE.AdditiveBlending, depthTest:false,transparent:true});//(1-(i/4))*0.2});	
		

		if(i==1 && paintsite() ) 	geom = new THREE.PlaneGeometry(200,200 * (115/460),1,1); 
...
//		if(i==1) 	geom = new THREE.PlaneGeometry(280,280 * (107/529),1,1); 
		...
		if(i>0 && paintsite()) logoMaterials.push(material); 
...
CountDownCube = function (numSides) {
	...
	var material = new THREE.MeshBasicMaterial( { color: lineColour, wireframe:true, wireframeLinewidth:lineWidth*8, opacity:lineOpacity*0.5, blending :THREE.AdditiveBlending, depthTest:false,transparent:true} ) ;
	...
	var material = new THREE.MeshBasicMaterial({map : THREE.ImageUtils.loadTexture(ImageFolder+numImgName), blending :THREE.AdditiveBlending, depthTest:false,transparent:true});
	...
	for(var i=-1; i<numSides+1; i++) { 
		...
		var uvs = planeGeom.faceVertexUvs[0][0]; 
 		for(var j=0; j<uvs.length; j++) {
 			var xy = uvs[j]; 
			uvs[j].x = (((i<0)?i+numSides:(i%numSides))*0.1)+(uvs[j].x*0.1); 
...
function setupRenderer() { 
	...
	if(Detector.webgl) {
		...
		renderer.setClearColor(0x040006);
		setInterval(loop, 1000/30);
		if (!paintsite || paintsite())
			return;
	...
	document.getElementById('cubeclock').appendChild(renderer.domElement);

	var canvas = renderer.domElement;

	canvas.style.background = "#2F2F2F";
...
function loop() {
	...
	// 	diff.sub(logoObject3D.position); 
	// 	diff.multiplyScalar(0.2); 
	// 	logoObject3D.position.add(diff); 
...
CountDownCube.prototype.updateSpin = function() {
	...
	if(this.scale.x!=this.targetScale) {
		...
		if(Math.abs(diff)<0.001) 	
			...
			this.scale.add(new THREE.Vector3(diff,diff,diff)); 
...
function onMouseUp(event) { 
	...
	else if(targetY/yMove<-1) targetY = -yMove; 
...
function makeSnow() { 
	...
	if(Detector.webgl) { 
		...
		var material = new THREE.ParticleBasicMaterial( { size: 15, blending: THREE.AdditiveBlending, depthTest: false, transparent : true, map:THREE.ImageUtils.loadTexture( ImageFolder+'snowparticle.png')  } );
